...
for(i in 1:length(lasFiles)){
  ...
  file %<>% sub(pattern = '_sensorMsg_slam', replacement = '', fixed = T)
  ...
  rotationData = rotateCloud(tempFile, '~/LAStools/bin/')
  ...
  writeLAS(lasRot, paste0('LAZ/',rotFile))
  
  slamFile = sub('\\.txt$', '_rot.txt', slamFile)
  write.table(slam, paste0('LAZ/',slamFile), row.names = F, col.names = F)
}
